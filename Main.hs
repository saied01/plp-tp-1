...
testsEj3 =
  test
    [       indentar 2 vacio ~?= vacio,
      ...
      indentar 2 (texto "a" <+> linea <+> texto "b")
          ~?= texto "a" <+> indentar 2 (linea <+> texto "b"),
      indentar 2 (linea <+> texto "a")
          ~?= indentar 1 (indentar 1 (linea <+> texto "a")),
  ...
  test
    [ mostrar vacio ~?= "",
      ...
      mostrar (indentar 2 (texto "a" <+> linea <+> texto "b" <+> linea <+> texto "c"))
        ~?= "a\n  b\n  c",
      mostrar (indentar 2 (indentar 3 (texto "a" <+> linea <+> texto "b"))) ~?= "a\n     b"
    ]
...
testsEj9 =
  test
    [ mostrar (pponADoc pericles) ~?= "{ \"nombre\": \"Pericles\", \"edad\": 30 }",
      ...
      -- un objeto compuesto de dos elementos
      mostrar (pponADoc addams) ~?= "{\n  \"0\": { \"nombre\": \"Pericles\", \"edad\": 30 },\n  \"1\": { \"nombre\": \"Merlina\", \"edad\": 24 }\n}",

      -- un objeto compuesto con nombres como claves
